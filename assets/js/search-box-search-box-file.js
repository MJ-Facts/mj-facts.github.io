...
async function initSearchIndex() {
  try {
    ...
    searchIndex = lunr(function () {
      ...
      console.log("initSearchIndex A-OK");
    });
  } catch (e) {
    console.log(e);
  }
...
function displayErrorMessage(message) {
  ...
  document.querySelector(".search-error").classList.add("y665");
}
...
function updateSearchResults(query, results) {
  ...
  document.querySelector(".search-results ul").innerHTML = results
    ...
    <li class="search-result-item" data-score="${hit.score.toFixed(2)}">
      <a href="${hit.href}" class="text-primary fs-5 fw-bolder">${hit.title}</a>
...
function showSearchResults() {
  ...
  document
    .getElementById("clear-search-results-modal")
...
function scrollToTop() {
  const toTopInterval = setInterval(function () {
    ...
    if (supportedScrollTop.scrollTop > no) {
